...
class WorkspaceService:
    ...
    async def create_lease(self, request: WorkspaceLeaseRequest) -> WorkspaceLease:
        ...
        if existing is not None:
            return await self._replay_existing(existing, request)
    ...
    async def _replay_existing(
        self, existing: WorkspaceLease, request: WorkspaceLeaseRequest
    ) -> WorkspaceLease:
        ...
        if (
            restore is None
            or snapshot.repository_id != existing.source_repository_id
            or snapshot.head_revision != restore.source_head_revision
            or snapshot.index_tree != restore.source_index_tree
            or snapshot.status_hash != restore.source_status_hash
        ):
    ...
    async def _create_lease(self, request: WorkspaceLeaseRequest) -> WorkspaceLease:
        ...
        try:
            if request.source_root is None:
                ...
                if staging_snapshot.head_revision != base_revision or not staging_snapshot.clean:
    ...
    async def _cleanup_path(self, destination: Path, source_location: GitLocation | None) -> None:
        if source_location is not None:
            await self._blocking(
                self._git.remove_worktree, source_location, destination, force=True
            )
    ...
    async def _capture_change_set(self, lease_id: str, *, run_id: str) -> CanonicalChangeSet:
        ...
        if (
            before.repository_id != restore.source_repository_id
            or before.head_revision != restore.source_head_revision
            or before.index_tree != restore.source_index_tree
            or before.status_hash != restore.source_status_hash
        ):
    ...
    def _valid_staging_snapshot(lease: WorkspaceLease, snapshot: GitRepositorySnapshot) -> bool:
        return (
            snapshot.work_tree == Path(lease.staging_root).resolve()
    ...
    async def close_lease(self, lease_id: str, *, run_id: str) -> WorkspaceLease:
        ...
        async with lock:
            ...
            try:
                await self._cleanup_path(Path(lease.staging_root), source_location)
            except asyncio.CancelledError:
                self._storage.transition_workspace_lease(
                    lease.id,
