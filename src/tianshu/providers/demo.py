...
def _rubric_payload(digest: str) -> str:
    return json.dumps(
        {"score": 1.0, "reasoning": f"{DEMO_MARK} deterministic rubric pass {digest}"},
        ensure_ascii=False,
        sort_keys=True,
    )
...
def _build_response(messages: list[dict], tools: list[dict] | None) -> LLMResponse:
    ...
    if _RUBRIC_MARKER in text:
        return LLMResponse(
            content=_rubric_payload(digest),
            tool_calls=None,
            usage=_zero_usage(),
            finish_reason="stop",
        )
