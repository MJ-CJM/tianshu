    ...
    async def _get_opinion(
        ...
        *,
        ...
        if self._agent is not None and edict is not None:
            # 工具是辅助,不是发言的前提。真实踩到过:某官员三次搜索连续返回空,
            # 便一言不发地退出,只留下一张没有正文的空卡片(issue #59)。
            user_prompt += (
                "\n【关于查证】你可用只读工具核实事实(网页搜索/抓取、翻阅代码与记忆)。"
                "搜索时有落空,可改用直接抓取已知官网、仓库页面。\n"
                "**无论查证成败,最终都必须按上述格式给出意见**:查不到就凭既有认识议,"
                "并在 OPINION 里说明哪些未能查证、结论因此有多少不确定。切勿因为查不到就不发言。\n"
            )
        if is_censor:
            user_prompt += (
