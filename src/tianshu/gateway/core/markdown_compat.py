"""markdown 兼容层：飞书/telegram 出站编排共享（core/outbound.py 复用）。

飞书的 post(tag=md) 与卡片 markdown 元素都不支持 GFM 表格语法
（| col | col |），渲染会被吃掉。本模块把表格转成"项: 值" 形式的列表
并提供超长内容的安全分段；telegram 出站编排经 OutboundEventBase 同样复用。

原位于 feishu/markdown_compat.py（历史遗留），现迁入 core 以消除
core/outbound.py → feishu 的潜伏循环 import；feishu 包保留同名
re-export shim 维持既有导入路径不变。
"""
