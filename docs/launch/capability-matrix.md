...
| 插件 manifest 目录 | Beta (read-only preview) / Beta（只读预览） | 只读 | 本地 manifest discovery 与 API/Web 展示 | 所有记录统一标 `manifest_only`、`loaded=false`；安装/激活返回 501；P1 典制影子归因不改变 catalog-only 边界 | 不安装、不 import、不执行、不卸载插件代码；当前 manifest catalog 也不是完整 PluginSet 依赖锁 | [`plugin API tests`](../../tests/gateway/test_plugin_manifest_api.py)；[`current boundary`](../design/self-evolving-agent-os/current-plugin-state.md)；[`implementation`](../../src/tianshu/plugins/) | deferred |
...
| Lean Core evolution | Experimental / 实验 | Candidate path available；SystemSnapshot shadow on | 单节点 Skill candidate、受控 canary；新受管 attempt 的典制影子归因 | candidate/evidence-bound Gate、Skill Promotion adapter、真实 assignment/effective overlay 与回滚已 `implemented`；P1 另以 V31 insert-once binding 与 required Evidence artifact 归因 kernel、executor manifest、skills、personas、policy、provider 及 governed overlay，并由 assignment/Edict 只读投影 | 典制写失败在影子期不阻断 run；dependency lock 仍为零值、policy 仅 id/priority、prompts 未填；无 RuntimeGeneration、生产 activation adapter、动态插件加载；OpenHands、ROI、cost calibration、full G4 为 `external_pending` | [`snapshot model/resolver tests`](../../tests/evolution/test_system_snapshot_resolver.py)；[`binding tests`](../../tests/universe/test_snapshot_binding.py)；[`Evidence tests`](../../tests/evidence/test_system_snapshot_artifact.py)；[`S5 Gate`](../cc-fable-v1/reports/s5-lean-evolution-report.md) | P1 shadow → P6 strict；G4 Lean Core only |
...
| Keqing 外部 Claude Code/Codex/Pi CLI | Experimental / 实验，contained + experimental | Off；在“天工院 → 客卿馆”可发现 | 可选本地 adapter；CLI 凭据 self-managed | 独立工作目录、clean-env、外围 timeout、事后结果归一；状态接口每次请求读取本机安装版本，Web 在进入、每 15 秒及重新聚焦时同步；当前本地 Pi `0.83.0` 与已验证基线一致，离线 RPC `get_state` / `get_session_stats` 通过；已注册 Keqing adapter 的 manifest content hash 会进入 P1 典制 executor component | 典制只绑定 manifest 内容身份，不固定本机 CLI 二进制或运行代；不自动执行 CLI 升级；新版本漂移先显示“待兼容验证”；credential gateway 写入返回 409；Provider 侧硬成本上限未实现；不属于 Candidate 黄金路径 | [`executor digest seam`](../../tests/evolution/test_system_snapshot_sources.py)；[`workspace lifecycle`](../../tests/executor/test_executor_workspace_lifecycle.py)；[`status tests`](../../tests/gateway/test_keqing_status.py)；[`clean-env`](../../tests/security/test_clean_env.py) | P3 generation；deferred G4 work |
