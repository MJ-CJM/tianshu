...
feat_phase4 深度参考。落点见 `docs/superpowers/plans/2026-04-02-phase1-agent-loop-redesign.md` 与 `docs/impl/executor.md`。

**采纳点**：

1. **ExitReason 枚举**（9 种退出原因）—— 取代此前的 `bool success` + 错误字符串。明确区分 `completed` / `max_iterations` / `context_overflow` / `timeout` / `cancelled` / `hook_blocked` / `budget_exhausted` / `llm_error` / `output_truncated`。
...
6. **Streaming** —— `StreamCallback` protocol（`on_text_delta` / `on_tool_call_delta` / `on_finish`）+ `WebSocketStreamCallback` 桥到前端。
...
feat_phase4 深度参考。落点见 `docs/superpowers/plans/2026-04-09-hermes-inspired-enhancements.md`。代码头部注释标记 `Ported from hermes-agent`。
...
2. **分层 Context 构建** —— NanoBot 4 层（Identity → Bootstrap → Memory → Skills），天枢扩展为 8 层（+ Court / L1 Palace / Recent Activity / Task Context）。
