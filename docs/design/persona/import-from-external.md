...
参见 [domain-model.md §5 执行主体本体论](../domain-model.md#5-执行主体本体论百官内臣-vs-客卿外臣)。

用户提出两个选项：

- **选项 A（一次性 seeding）✓**：把外部来源的**人格 + 技能 + 提示词**收进来，种出一个天枢自己的**内臣**。该内臣此后**独立演化、受京察治理**。
...
2. **只导入可移植的人格 + 能力**，**排除运行态**（记忆 / 用户模型 / 学习历史 / 渠道 / 网关）。运行态属于「自进化 / 外部机构」，导进来会与天枢自进化冲突。
...
- 技能原生兼容：天枢 `SkillsLoader` 直接读 agentskills.io 的 `SKILL.md`，且已识别 `metadata.openclaw` 命名空间（`skills/loader.py`）。hermes/openclaw 技能即 agentskills.io SKILL.md，**几乎无转换层**。
...
- **`gateway/personas_api.py`**：
  ...
  - `create_persona` 扩展：`imported_soul` / `imported_role`（注入天枢 frontmatter 后写 SOUL/ROLE）+ `import_skill_paths`（经 `_install_imported_skills` 复制 SKILL.md 目录到 `runtime_skills_target()`，校验名字，收集入 `skills_allowed`；冲突 / 非法名跳过，失败安全）。

### 前端

- `PersonaFormModal`（`PersonaDashboardPage.tsx`）加「创建方式」Segmented：`模板库` / `从外部导入`。选「外部导入」→ 选 source + path → 「导入预览」→ 预填 name、SOUL/ROLE 预览（Collapse）、建议模型、技能勾选，并显示 `source_notes`。
- `api/personas.ts`：`previewPersonaImport(source, path?)`；`api/types.ts`：`PersonaImportDraft` / `PersonaImportSourceKind` + `PersonaCreateRequest` 加 `imported_soul?` / `imported_role?` / `import_skill_paths?`。
