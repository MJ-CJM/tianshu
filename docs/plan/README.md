...
| [Phase 1](./phase-1-governance.md) | 引入治理与异步调度 | EventBus + Scheduler + Planner + Auditor + Notifier + 人工复核 + **WebSocket 实时 + 批红台 + 事件时间线** + **CLI 治理指令** | 14 | Web 服务 + 事件驱动调度 |
| [Phase 2](./phase-2-platform.md) | 引入平台化能力 | Memory + CostManager + 多 Provider + 多通道通知 + PluginApi + **成本报表 + Provider/插件管理** + **CLI 平台指令** | 9 | 平台化 Web 服务 |
| [Phase 3](./phase-3-multi-agent.md) | 多 Agent 与分布式扩展 | DAG + 多 Agent 并发 + PostgreSQL + K8s + **DAG 作战图** + **CLI 分布式指令** | 9 | 容器集群 / K8s / Temporal |
...
| 内阁 | `Planner` | 任务拆解与规划 | Phase 1 |
| 都察院 | `Auditor` | 审计与风控 | Phase 1 |
| 通政司 | `Notifier` | 渲染与通知 | Phase 1 |
| — | `Decree`（批红） | 用户反馈与复核 | Phase 1 |
| 文渊阁 | `Memory` | 记忆与检索 | Phase 2 |
| 户部 | `CostManager` | Token/成本治理 | Phase 2 |
