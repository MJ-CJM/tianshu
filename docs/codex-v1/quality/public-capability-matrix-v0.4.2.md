...
> **历史能力快照（2026-07-12）：** 本表保留当时的公开边界，不再是当前事实源。
> 当前认证、持久治理、长程/定时任务和 Web 状态见
> [当前能力事实矩阵](../../launch/capability-matrix.md)。

> At the snapshot date, this was the public source of truth for capability claims. /
> 在该快照时点，本表是当时公开能力承诺的事实源。
...
| Web 与 IM 裁决入口 | Experimental / 实验 | Optional | Web；Telegram 按钮；飞书命令回复 | 当前进程存活期间可查看并提交受支持的人工裁决 | 飞书不是交互按钮卡片；等待态不耐重启；不是原生移动端产品 | [`PendingView.tsx`](../../../web/src/components/study/PendingView.tsx)；[`decrees.ts`](../../../web/src/api/decrees.ts)；[`Feishu command tests`](../../../tests/gateway/feishu/test_approval_commands.py)；[`Telegram callback/button tests`](../../../tests/gateway/telegram/test_callback.py) | G2 |
