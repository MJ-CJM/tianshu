...
- 最终自动 G3 Gate 通过后先记录不可变的 `automation_passed` 证据，同时把 Gate 状态标记为 `user_approval_pending`；必须把真实页面交给用户人工审批，用户明确批准后才能标记 G3 `passed`。
...
- `docs/quality/g3-web-gate.md` records exact commands, environment, actual counts/metrics, immutable `automation_passed` evidence, and Gate status `user_approval_pending` after a green run.
