...
覆盖 `src/tianshu/persona/` 全部模块 + `personas/`（git 模板）+ `~/.tianshu/personas/`（运行时）+ `templates/persona/`（vendored 模板库）。
...
| 部门模型 | `persona/department.py` | `Department` |
...
| 层 | 路径 | 写入方 |
|---|---|---|
| 模板 | `personas/{department}/`（SOUL/ROLE/MEMORY） | 代码仓库（git） |
| 运行时 | `~/.tianshu/personas/{id}/`（SOUL/ROLE） | UI / API |

`PersonaLoader.load_all()`：有 storage 时 `_seed_from_files()`（no-op，仅作模板源）+ `_load_from_db()` 填内存缓存；无 storage 时 `_load_from_files()`（legacy）。

`_dict_to_persona(d)`：定位部门模板目录（`personas/{department}/`，回退 `personas/{id}/`）→ `ensure_runtime_identity` 拷 SOUL/ROLE 到运行时（幂等，已存在不覆盖）→ DB 存的 path 被忽略，运行时目录权威。`delete()` 同时删 DB + 缓存 + 文件目录（防 seed 复活）。`repoint_runtime()` 在位面切换时切运行时根目录并重载。
...
`TemplateLibrary.load()` 扫 `templates/persona/{zh,en}/{category}/*.md`（跳过 README）。`split_template` 按 `_MISSION_MARKERS` 标题切 SOUL/ROLE，无命中走 intro/body fallback。`render` 返回 `(soul_md, role_md)`。

## 7. 扩展点

- **新官员**：在 `personas/{dept}/` 放 SOUL/ROLE/MEMORY 或经 UI 从 TemplateLibrary seed + `loader.save()` 写 DB。
...
`bingbu / ducha / hubu / neige / tongzheng / wenyuan` 六部模板（各含 SOUL.md / ROLE.md / MEMORY.md）+ `court/`（COURT.md + MEMORY.md，非独立 persona）。运行时 SOUL/ROLE 副本在 `~/.tianshu/personas/{id}/`，运行时记忆在 `~/.tianshu/memory/{id}/`。
