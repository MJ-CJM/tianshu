...
export async function seedClosedEvidence(
  ...
  const script = [
    ...
    "    unit_of_work.connection.execute('UPDATE memorials SET status=?, review_status=?, completed_at=?, audit_json=? WHERE id=?', (completed_memorial.status.value, completed_memorial.review_status, completed_memorial.completed_at.isoformat(), completed_memorial.audit.model_dump_json(), memorial.id))",
    ...
    "    opened = service._build_open_current(unit_of_work.connection, memorial.id)",
    "    service._close_current(unit_of_work.connection, memorial.id, expected_version=opened.version)",
    "    unit_of_work.commit()",
    "storage.close()",
  ].join("\n");
