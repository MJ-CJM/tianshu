...
export async function seedClosedEvidence(
  ...
  const script = [
    ...
    "    unit_of_work.connection.execute('UPDATE memorials SET status=?, completed_at=?, audit_json=? WHERE id=?', (completed_memorial.status.value, completed_memorial.completed_at.isoformat(), completed_memorial.audit.model_dump_json(), memorial.id))",
