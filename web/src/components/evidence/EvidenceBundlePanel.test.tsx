...
describe("EvidenceBundlePanel", () => {
  it("shows separated evidence, digest, download and governed replay warning", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "执行器身份" }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("重放会创建新的受治理敕令，不会在浏览器执行原命令。"),
    ).toBeInTheDocument();
  });
  ...
  it("shows open status and never exposes a download control before closure", () => {
    render(
      <EvidenceBundlePanel
        bundle={{
          ...
          artifacts: [],
          checks: [
            { name: "unit tests", status: "unavailable", exitCode: null },
          ],
          policies: [],
          cost: "¥0.00",
          environment: [],
    ...
    expect(
      screen.queryByRole("link", { name: "下载证据包" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "受治理重放" }),
    ).not.toBeInTheDocument();
  });
});
