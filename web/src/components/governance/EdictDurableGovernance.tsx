...
const OUTER_LOOP_ACTIONS: readonly DecisionAction[] = [
  "continue",
  "accept_as_is",
  "abort",
];
...
function evidenceView(
  ...
  const run = detail.runs.find(
    ({ memorial_id }) => memorial_id === evidence.memorial_id,
  );
...
export default function EdictDurableGovernance({
  ...
  const latestEvidence = latestRun
    ? (detail.evidence.find(
        ({ memorial_id }) => memorial_id === latestRun.memorial_id,
      ) ?? null)
    : null;
  ...
  const capabilities: GovernanceCapabilityView[] = (
  ...
  const mandatoryMismatches = capabilities
    .filter(
      ({ requested: mode, effective: state }) =>
        mode === "mandatory" && state !== "enforced",
    )
  ...
  return (
    ...
    >
      ...
      <GlowCard
        title={t("page.edictDetail.durable.runStateTitle")}
        style={{ marginBottom: 24 }}
      >
        {detail.runs.length === 0 ? (
          <Typography.Text type="secondary">
            {t("page.edictDetail.durable.runsEmpty")}
          </Typography.Text>
        ) : (
          detail.runs.map((run) => (
            <Descriptions
              ...
              items={[
                {
                  key: "memorial",
                  label: t("page.edictDetail.durable.memorial"),
                  children: <code>{run.memorial_id}</code>,
                },
                {
                  key: "phase",
                  label: t("page.edictDetail.durable.phase"),
                  children: <Tag>{run.phase}</Tag>,
                },
                {
                  key: "version",
                  label: t("page.edictDetail.durable.version"),
                  children: run.version,
                },
                {
                  key: "cursor",
                  label: t("page.edictDetail.durable.sideEffectCursor"),
                  children: run.side_effect_cursor,
                },
                {
                  key: "checkpoint",
                  label: t("page.edictDetail.durable.checkpoint"),
                  children: run.checkpoint_present
                    ? t("page.edictDetail.durable.yes")
                    : t("page.edictDetail.durable.no"),
                },
                {
                  key: "lineage",
                  label: t("page.edictDetail.durable.planLineage"),
                  children:
                    run.plan_lineage.length > 0 ? (
                      <Space wrap>
                        {run.plan_lineage.map((revision) => (
                          <code key={revision.revision_id}>
      ...
      <GlowCard
        title={t("page.edictDetail.durable.decisionsTitle")}
        style={{ marginBottom: 24 }}
      >
        {detail.decisions.length === 0 ? (
          <Typography.Text type="secondary">
            {t("page.edictDetail.durable.decisionsEmpty")}
          </Typography.Text>
        ) : (
          detail.decisions.map((decision) => (
            <article
              key={decision.request.decision_request_id}
            ...
            >
              ...
              {Object.keys(decision.request.payload).length > 0 ? (
                <dl>
                  {Object.entries(decision.request.payload).map(
                    ([key, value]) => (
                      <div key={key}>
                        <dt>{key}</dt>
                        <dd>
              ...
              <DecisionPanel
                decision={{
                  ...
                  resolvedAction: decision.resolution?.action as
      ...
      <GlowCard
        title={t("page.edictDetail.durable.evidenceTitle")}
        style={{ marginBottom: 24 }}
      >
        {detail.evidence.length === 0 ? (
          <Typography.Text type="secondary">
            {t("page.edictDetail.durable.evidenceEmpty")}
          </Typography.Text>
        ) : (
