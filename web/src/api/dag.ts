...
export async function getDagByEdict(
  edictId: string,
): Promise<ApiResponse<DAGExecution> | null> {
  try {
    const { data } = await apiClient.get(`/dag/by-edict/${edictId}`, {
      silentCodes: [404],
    } as any);
    return data;
  } catch (err: unknown) {
...
export async function getDag(
  dagId: string,
): Promise<ApiResponse<DAGExecution>> {
  const { data } = await apiClient.get(`/dag/${dagId}`);
  return data;
}

export async function cancelDag(
  dagId: string,
): Promise<ApiResponse<{ dag_id: string; cancelled_nodes: string[] }>> {
  const { data } = await apiClient.post(`/dag/${dagId}/cancel`);
  return data;
}

export async function retryDag(
  dagId: string,
  fromNodeIds?: string[],
): Promise<ApiResponse<{ dag_id: string; reset_node_ids: string[] }>> {
