...
def test_apply_rewrites_persona_file(tmp_path):
    ...
    r = asyncio.run(apply_mutation(store, "child",
        {"target": "persona:bingbu/ROLE.md", "reason": "更主动"}, _llm("新职责：主动协同")))
...
def test_missing_file_no_op(tmp_path):
    store = _store(tmp_path)
    r = asyncio.run(apply_mutation(store, "child",
        {"target": "persona:bingbu/SOUL.md", "reason": "x"}, _llm("y")))
...
def test_no_change_not_applied(tmp_path):
    store = _store(tmp_path)
    r = asyncio.run(apply_mutation(store, "child",
        {"target": "persona:bingbu/ROLE.md", "reason": "x"}, _llm("原始职责：被动执行")))
    assert r["applied"] is False
...
def test_oversized_rewrite_rejected(tmp_path):
    ...
    huge = "x" * (64 * 1024 + 1)
    r = asyncio.run(apply_mutation(store, "child",
        {"target": "persona:bingbu/ROLE.md", "reason": "x"}, _llm(huge)))
    assert r["applied"] is False
